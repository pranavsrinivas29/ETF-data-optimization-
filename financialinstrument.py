...
class FinancialInstrument():
    ...
    interval: str
        frequency, valid intervals: 1m,2m,5m,15m,30m,60m,90m,1h,1d,5d,1wk,1mo,3mo
    ...
    def __repr__(self): 
        return "FinancialInstrument(ticker = {}, start = {}, end = {}, interval = {})".format(self._ticker, 
                                                                               self.start, self.end, self.interval)
    def get_data(self):
        ''' retrieves (from yahoo finance) and prepares the data
        '''
        #raw = yf.download(self._ticker, self.start, self.end, interval = self.interval).Close.to_frame()
        
        raw = yf.download(self._ticker, self.start, self.end, interval=self.interval)
        print(raw)
